var chooseTime = function() {
	...
	self.GetData = function() {
		var self = this;
		var ajaxUrl = common.gServerUrl + 'API/Teacher/GetFreetimeRemain?userid=107';
	...
	mui.plusReady(function() {
		...
		if( typeof(web.chosenTimes) !== "undefined") {
			self.ChosenTimes(web.chosenTimes);
		}
		//直接跳转到该页面的参数：日期数组
		self.GetData();
	})
	...
	self.back = function() {
		...
		if (self.ChosenTimes().length <= 0) {
			...
			common.transfer('aboutLesson.html', true, {
				chosenTimes: self.ChosenTimes()
			},true);
			//ToDo：此处应该将self.ChosenTimes()返回到之前页面
		}
	}
	...
	self.initCell = function(date, hour) {
		...
		self.Freetimes().forEach(function(freetime) {
			if (self.WeekIndex() == freetime.WeekIndex && date.getDay() == freetime.DayOfWeek &&
				...
				ret = new Date(date.getYear() + 1900, date.getMonth(), date.getDate());
				ret.setHours(hour);
				ret.setMinutes(0);
				ret.setSeconds(0);
